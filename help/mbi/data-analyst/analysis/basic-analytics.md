...
* **[!UICONTROL Customer's lifetime number of coupons]**: Die Gesamtzahl der von einem Kunden während seiner Lebensdauer verwendeten Gutscheine.
...
*Wenn ja, enthält diese Tabelle möglicherweise nicht alle Ihre Kunden. Wenden Sie sich an den [Supportteam](https://experienceleague.adobe.com/docs/commerce-knowledge-base/kb/troubleshooting/miscellaneous/mbi-service-policies.html) , um sicherzustellen, dass Ihre Kundenanalysen alle Kunden einschließen.*

*Sind Sie nicht sicher, ob Sie Gastbestellungen annehmen? Siehe Abschnitt [dieses Thema](../data-warehouse-mgr/guest-orders.md) um mehr zu erfahren!*
...
* **[!UICONTROL Created_at]**: Das Datum der Erstellung oder Platzierung der Bestellung.
...
* **[!UICONTROL Order_id]**: Der Referenzschlüssel für Ihre `Orders` gibt an, welche Artikel in derselben Bestellung gekauft wurden. Wenn eine Bestellung mehrere Elemente enthält, wird dieser Wert wiederholt.
* **[!UICONTROL Product_id]**: Wenn Sie Informationen zum gekauften Produkt erhalten möchten (z. B. Farbe, Größe usw.), können Sie diese Informationen in dieser Spalte aus Ihrer Produkttabelle abrufen.
* **[!UICONTROL Order's created_at]**: Der Zeitstempel, mit dem die Bestellung aufgegeben wurde, in der Regel nach `order line items` aus der Tabelle `Orders` Tabelle.
* **[!UICONTROL Order's coupon_code]**: Ähnlich wie bei der `Order's created_at` aus, wird diese Spalte aus Ihrer Auftragstabelle kopiert.
...
* **[!UICONTROL Spend]**: Die Gesamtausgaben der Anzeige. In [!DNL Facebook], wäre dies die Ausgabenspalte im `facebook_ads_insights_####` Tabelle. Für [!DNL Google AdWords], würde dies `adCost` in der `campaigns####` Tabelle.
...
* **[!UICONTROL Impressions]**: Die Gesamtanzahl der Impressionen. In [!DNL Facebook], wären dies die Impressionen in der `facebook_ads_insights_####` Tabelle. In [!DNL Google AdWords], wären dies die Impressionen der `campaigns####` Tabelle.
...
* **[!UICONTROL Date]**: Die Zeit und das Datum, an dem die Aktivität (Ausgaben, Klicks oder Impressionen) für eine bestimmte Kampagne aufgetreten ist. In [!DNL Facebook], würde dies `date_start` in der `facebook_ads_insights_####` Tabelle. In [!DNL Google AdWords], wäre dies die Datumsspalte in der `campaigns####` Tabelle.
...
* **Berichtsbeispiel**: Die 25 beliebtesten Kunden nach der Lebensdauer der Bestellungen
   ...
   * **[!UICONTROL Show Top/Bottom]**: Die 25 beliebtesten Sortierungen nach Bestellungen
...
* **Berichtsbeispiel**: Top 25 Kunden nach Lifetime-Umsatz
   ...
   * **[!UICONTROL Time Interval]**: `None`
   * **[!UICONTROL Group By]**: `customer_email`
   * **[!UICONTROL Show Top Bottom]**: Die 25 beliebtesten nach Lebensdauerumsatz sortiert
...
* **Berichtsbeispiel**: Durchschnittlicher Kundenlebenszeitumsatz nach Kohorte
   ...
   * **[!UICONTROL Time Period]**: Verschieben des Sets von Kohorten der letzten acht Kohorten mit mindestens vier Monaten Daten
...
* **Berichtsbeispiel**: Coupon- und Nicht-Coupon-Kunden nach Monat
   ...
   * **[!UICONTROL Metric]**: `New Users`
   * **[!UICONTROL Filters]**: Kundenlebensdauer Anzahl der Bestellungen größer als 0 und Kundenlebensdauer Anzahl der Coupons größer als 0
   * **[!UICONTROL Time range]**: `All Time`
   * **[!UICONTROL Time interval]**: `By Month`
...
* **Berichtsbeispiel**: Produkte pro Bestellung: Erstbestellung vs. Wiederholungen von Bestellungen
   * **[!UICONTROL Metric A]**: Artikel bestellt: erste Bestellung
      * **[!UICONTROL Metric]**: `Products ordered`
...
* **Berichtsbeispiel**: Fügen Sie zunächst die `New customers` Metrik. Fügen Sie dann die `Average lifetime revenue` Metrik. Wählen Sie den gewünschten Zeitraum aus und wählen Sie die `interval` as `None`. Wählen Sie abschließend die `group by` Option als`Customer's first order's campaign`.
   * **[!UICONTROL Metric A]**: `New Customers`
