...
Mit MBI können Sie [Daten zu den Kosten und Umsatzdaten von Ehewerbung](../../data-analyst/importing-data/integrations/google-adwords.md) aus Ihrer Datenbank. Auf diese Weise können Sie feststellen, welche Kampagnen den höchsten ROI haben. In diesem Artikel werden verschiedene Methoden zur Bewertung der Kampagnenleistung beschrieben.
...
* Importieren Sie Ihre Daten zu Werbekosten:
   ...
   * Wenn Sie Kostendaten aus mehreren Quellen importieren, können wir [Konsolidierung](../../best-practices/consolidating-your-tables.md) die Daten in [!DNL MBI]. Einfach [Support-Ticket einreichen](../../guide-overview.md).
...
Die Analysen (1) und (2) oben werden in einem separaten Tutorial zu [die wichtigsten Marketing-Kanäle identifizieren](../analysis/most-value-source-channel.md). In unserem Beispiel analysieren wir die Analyse (3), um den ROI der Kampagne im Zeitverlauf zu messen. Auf diese Weise wird ermittelt, ob Benutzer, die von einer bestimmten Kampagne erworben wurden, genügend Umsatz während der Lebensdauer generiert haben, um ihre Akquisekosten zu decken.

>[!NOTE]
>
>Wir gehen davon aus, dass alle Kampagnenkosten ausschließlich für den Erwerb neuer Nutzer verwendet wurden. In der Realität werden Ihre Kampagnenkosten auch mit dem Erwerb nicht konvertierter Besuche, Wiederholungskäufern usw. geteilt. Wenn man jedoch davon ausgeht, dass alle Kosten für den Erwerb neuer registrierter Benutzer verwendet werden, wird der daraus resultierende ROI das Worst-Case-Szenario (höchste Kosten pro Akquisition) berücksichtigen, sodass Sie sicher sein können, dass Ihr tatsächlicher ROI höher ist als unsere Berechnung.
>
>Beispiel: Angenommen, Sie haben 20 USD für eine Kampagne ausgegeben, die 10 neue Benutzer und 10 wiederkehrende Käufer generiert hat, belaufen sich Ihre tatsächlichen Kosten pro neuer Benutzer auf 1 USD, aber unter unserer Annahme, dass alle Kosten für die Akquise neuer Benutzer anfallen, belaufen sich die Kosten pro Akquise auf 2 USD.)
...
1. wir sind fertig. Klicken `Back to Metric List` oben und gehen Sie zu einem beliebigen Dashboard.
...
1. Wählen Sie die [!UICONTROL Adword Cost] Metrik, die wir gerade erstellt haben
...
1. Dieser Bericht zeigt Ihre [!DNL AdWords] Kosten nach Kampagnen

**2. Anschließend erstellen wir einen Bericht, in dem neue Benutzer nach Kampagnen gezählt werden:**
...
1. Dieser Bericht zeigt Ihre bisher registrierten Benutzer nach Kampagnen

**3. Erstellen Sie außerdem einen Bericht, der die durchschnittliche LTV-Nutzung nach Kampagnen segmentiert:**
...
1. Dieser Bericht zeigt Ihren durchschnittlichen Benutzerlebenszeitumsatz nach Kampagnen

**Schließlich berechnen wir den Kampagnen-ROI, indem wir diese drei Analysen in einem Bericht zusammenführen:**

1. Klicken Sie in einem beliebigen Dashboard auf **[!UICONTROL Add Report > Create new report]**
1. Fügen Sie als Eingabe hinzu, verwenden wir die drei oben verwendeten Metriken. Jedem wird ein Brief zugewiesen (z. B.\[`A`\], \[`B`\] und \[`C`\])
1. [!UICONTROL Cost]: Fügen Sie die Metrik AdWords-Kosten hinzu - dies ist eine Variable \[A\]. Dadurch werden Kosten einfach durch Kampagnen zurückgegeben.
1. [!UICONTROL Users]: Fügen Sie die Metrik Neue Benutzer hinzu - dies ist die Variable \[B\]. Dadurch wird einfach die Anzahl der Benutzer nach Kampagnen zurückgegeben.
1. [!UICONTROL LTV]: Fügen Sie die Metrik Durchschnittlicher Lebensdauerumsatz hinzu - dies ist eine Variable \[`C`\]. Dadurch wird LTV einfach per Kampagnen zurückgegeben.

1. Klicken Sie auf das Symbol zum Ausblenden neben dem Wortdiagramm, damit Sie sich auf die Tabelle konzentrieren können.
1. Jetzt verwenden wir `Add Formula` , um diese Metriken wie folgt zu kombinieren:
...
1. [!UICONTROL Avg Return per User]: Formel eingeben **\[`C`\]-(\[`A`\]/\[`B`\])**. Dadurch wird die durchschnittliche Spanne zurückgegeben, die für einen Benutzer durch Berechnung (durchschnittliche LTV-Nutzer) - (durchschnittliche Kosten pro Akquise) ermittelt wurde.
...
**2. Um den ROI zu berechnen, kombinieren wir erneut Metriken in einem Bericht:**
...
1. Fügen Sie nun die [!DNL AdWords Cost] Metrik, folgen Sie dem Filter und der Gruppe nach Anweisungen aus dem `Ad cost by campaigns` Bericht, den wir im `User acquisition campaigns` Abschnitt oben; Klicken Sie dann auf **[!UICONTROL Hide]** unterhalb des Skalarwerts der Metrik
1. Mit diesen Metriken fügen wir nun Formeln hinzu:
