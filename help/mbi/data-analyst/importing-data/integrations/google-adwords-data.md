...
Dort sind zwei Tabellen für die Replikation in Ihrer Data Warehouse verfügbar: `campaigns[account-id]` und `adwords[account-id]`.
...
Adobe empfiehlt die Erstellung einer `consolidated ad spend` -Tabelle verwenden, um die Daten aus all Ihren verschiedenen Werbequellen in einer einzigen Tabelle zu kombinieren. Auf diese Weise können Sie einen einzigen Metriksatz für die Werbeanalyse verwenden.
